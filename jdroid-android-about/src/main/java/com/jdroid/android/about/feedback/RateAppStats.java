...
public class RateAppStats {
	...
	public static void setGiveFeedback(Boolean feedback) {
		...
		getSharedPreferencesHelper().savePreferenceAsync(LAST_RESPONSE_TIMESTAMP, DateUtils.INSTANCE.nowMillis());
	}
	...
	public static Boolean displayRateAppView() {
		...
		boolean enoughDaysSinceLastResponse = TimeUnit.MILLISECONDS.toDays(DateUtils.INSTANCE.nowMillis() - RateAppStats.getLastResponseTimestamp()) >= AbstractApplication.get().getRemoteConfigLoader().getLong(AboutRemoteConfigParameter.RATE_APP_MIN_DAYS_SINCE_LAST_RESPONSE);
		boolean enoughDaysSinceFirstAppLoad = TimeUnit.MILLISECONDS.toDays(DateUtils.INSTANCE.nowMillis() - UsageStats.getFirstAppLoadTimestamp()) >= AbstractApplication.get().getRemoteConfigLoader().getLong(AboutRemoteConfigParameter.RATE_APP_MIN_DAYS_SINCE_FIRST_APP_LOAD);
		...
		boolean enoughDaysSinceLastCrash = TimeUnit.MILLISECONDS.toDays(DateUtils.INSTANCE.nowMillis() - UsageStats.getLastCrashTimestamp()) >= AbstractApplication.get().getRemoteConfigLoader().getLong(AboutRemoteConfigParameter.RATE_APP_MIN_DAYS_SINCE_LAST_CRASH);
